...
const { Agama, brainly, Hallo, P, Pacaran, Perkenalan, Toxic, downloader, cekResi, removebg, urlShortener, meme, translate, getLocationData } = require('../../lib')
...
module.exports = msgHandler = async (client, message) => {
    try {
        ...
        case 'menuadmin':            
            ...
            await client.sendText(from, menuId.textAdmin(pushname))
            break
        case 'donate':
        case 'donasi':
            ...
            break
        case 'Hallo':
            await client.sendText(from, menuId.textHallo(pushname))
            break
       case 'Perkenalan':
            await client.sendText(from, menuId.textPerkenalan(pushname))
                .then(() => ((isGroupMsg) && (isGroupAdmins)) ? client.sendText(from, 'Menu Admin Grup: *#menuadmin*') : null)
            break
       case 'P':
            await client.sendText(from, menuId.textP(pushname))
                .then(() => ((isGroupMsg) && (isGroupAdmins)) ? client.sendText(from, 'Menu Admin Grup: *#menuadmin*') : null)
            break
       case 'Agama':
            await client.sendText(from, menuId.textAgama(pushname))
                .then(() => ((isGroupMsg) && (isGroupAdmins)) ? client.sendText(from, 'Menu Admin Grup: *#menuadmin*') : null)
            break
       case 'Pacaran':
            await client.sendText(from, menuId.textPacaran(pushname))
                .then(() => ((isGroupMsg) && (isGroupAdmins)) ? client.sendText(from, 'Menu Admin Grup: *#menuadmin*') : null)
            break
       case 'Toxic':
            await client.sendText(from, menuId.textToxic(pushname))
                .then(() => ((isGroupMsg) && (isGroupAdmins)) ? client.sendText(from, 'Menu Admin Grup: *#menuadmin*') : null)
            break
        // Sticker Creator
        case 'sticker':
        case 'stiker': {
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                * Check premium feature at https://trakteer.id/Dark-Knight/showcase or chat Author for Information.
                */
